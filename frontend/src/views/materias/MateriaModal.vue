                ...
                <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
                    <div class="space-y-1.5">
                        ...
                        <select v-model="formData.carrera_id" required :disabled="isSaving"
                            ...
                            :class="{ 'border-red-500 focus:ring-red-500': validationErrors?.carrera_id }">
                            <option :value="null" disabled>Selecciona una carrera...</option>
                            <option v-for="carrera in carreras" :key="carrera.id" :value="carrera.id">
                                {{ carrera.nombre }}
                            </option>
                        </select>
                        <p v-if="validationErrors?.carrera_id" class="text-xs text-red-500 mt-1 block">
                            {{ validationErrors.carrera_id[0] }}
                        </p>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select v-model="formData.semestre_academico_id" required :disabled="isSaving"
                            class="w-full px-3 py-2 bg-white border border-gray-200 text-gray-900 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm transition-all"
                            :class="{ 'border-red-500 focus:ring-red-500': validationErrors?.semestre_academico_id }">
                        ...
                        <p v-if="validationErrors?.semestre_academico_id" class="text-xs text-red-500 mt-1 block">
                            {{ validationErrors.semestre_academico_id[0] }}
                        </p>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input v-model="formData.codigo" type="text" required placeholder="Ej: MAT-101"
                            ...
                            :class="{ 'border-red-500 focus:ring-red-500': validationErrors?.codigo }" />
                        <p v-if="validationErrors?.codigo" class="text-xs text-red-500 mt-1 block">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input v-model="formData.creditos" type="number" placeholder="Ej: 4" :disabled="isSaving" min="0"
                            class="w-full px-3 py-2 bg-white border border-gray-200 text-gray-900 placeholder:text-gray-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm transition-all"
                            :class="{ 'border-red-500 focus:ring-red-500': validationErrors?.creditos }" />
                        <p v-if="validationErrors?.creditos" class="text-xs text-red-500 mt-1 block">
                            {{ validationErrors.creditos[0] }}
                        </p>
                    </div>
