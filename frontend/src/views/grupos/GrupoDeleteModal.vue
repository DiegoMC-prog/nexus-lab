    ...
    >
      <div class="bg-white rounded-xl shadow-xl border border-gray-100 max-w-md w-full overflow-hidden transform transition-all scale-100">
        ...
        <div class="p-6 flex gap-4">
          ...
          <div class="space-y-2">
            <h3 class="text-lg font-bold text-gray-900">¿Eliminar Grupo Académico?</h3>
            <p class="text-sm text-gray-500">
              ¿Estás seguro de que deseas eliminar permanentemente este grupo del sistema? Esta acción no se puede deshacer y retirará las asignaciones de horarios vinculadas.
            </p>
            
            <div v-if="grupo" class="bg-gray-50 p-3 rounded-lg border border-gray-200 text-xs font-medium space-y-1">
              <div class="text-gray-950 font-bold text-sm">{{ grupo.nombre }}</div>
              <div class="text-gray-500 font-mono">Materia: {{ grupo.nombre_materia }}</div>
              <div class="text-gray-500">Gestión/Período: {{ grupo.gestion }}</div>
            </div>
          </div>
        </div>
