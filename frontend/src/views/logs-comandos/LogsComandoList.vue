...
import { 
    ...
    Terminal, User, Laptop, Eye, HelpCircle, 
    CheckCircle, XCircle 
} from '@lucide/vue';
...
<template>
    <div class="p-6 space-y-6 relative">
        ...
        <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="relative md:col-span-2">
                    ...
                    <input v-model="searchTerm" type="text" placeholder="Buscar por origen, respuesta, usuario o estación..."
                        ...
                        <tr v-for="log in logs" :key="log.id" class="hover:bg-gray-50/70 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-3xs text-gray-400 font-mono tracking-tight">{{ log.usuario?.email }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <div>
                                        <p class="text-slate-800 font-semibold font-mono">{{ log.estacion?.hostname }}</p>
                                        <p class="text-3xs text-gray-400 font-mono tracking-tight">{{ log.estacion?.direccion_ip }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <span :class="[
                                    ...
                                    <component :is="log.estado === 'exitoso' || log.estado === 'success' ? CheckCircle : XCircle" class="w-3 h-3 shrink-0" />
                                    {{ log.estado }}
                                </span>
                            </td>
        ...
        >
            <div v-if="isViewerOpen" class="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
                <div class="bg-white rounded-xl border border-gray-100 max-w-lg w-full shadow-2xl relative overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-150">
                    ...
                    <div class="px-6 pt-6 pb-4 flex items-start justify-between border-b border-gray-100">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-400 mt-0.5 font-mono">ID Transacción: #{{ selectedLog?.id }}</p>
                            </div>
                        </div>
                    ...
                    <div class="p-6 space-y-4">
                        <div class="space-y-1">
                            ...
                            <p class="text-xs text-gray-800 leading-relaxed font-semibold">
                                El usuario <span class="text-blue-600">{{ selectedLog?.usuario?.name }}</span> envió el comando <span class="font-mono bg-slate-100 px-1 py-0.5 rounded font-bold">{{ selectedLog?.comando?.nombre }}</span> a la máquina <span class="font-mono font-bold">{{ selectedLog?.estacion?.hostname }}</span>.
                            </p>
                        </div>
                        
                        <div class="space-y-1">
                            <h4 class="text-3xs font-bold text-gray-400 uppercase tracking-widest">Terminal / Sensor de Respuesta</h4>
                            <div class="bg-slate-950 text-slate-100 font-mono text-2xs p-4 rounded-lg overflow-x-auto shadow-inner leading-relaxed select-all">
                                <p v-if="selectedLog?.mensaje_respuesta" class="whitespace-pre-line">{{ selectedLog.mensaje_respuesta }}</p>
                                <p v-else class="text-slate-500 italic">Ningún mensaje de respuesta devuelto por el agente de NexusLab.</p>
                            </div>
                        </div>
                    </div>
